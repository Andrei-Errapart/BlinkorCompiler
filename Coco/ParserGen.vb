...
Namespace at.jku.ssw.Coco
	...
	Public Class ParserGen
		...
		Private Sub GenErrorMsg(ByVal errTyp As Integer, ByVal sym As Symbol)
			errorNr += 1
			err.Write(StrDup(4, vbTab) & "Case {0,4} : ", errorNr)
			err.Write("s = """)
			Select Case errTyp
				Case tErr
		...
		Private Sub OpenGen(ByVal backUp As Boolean) ' pdt
			Try
				Dim fn As String = Path.Combine(tab.outDir, "Parser.vb") ' pdt
		...
		Public Sub WriteParser()
			...
			Dim fr As String = Path.Combine(tab.srcDir, "Parser.frame")
			If Not File.Exists(fr) Then
				If tab.frameDir IsNot Nothing Then
					fr = Path.Combine(tab.frameDir.Trim(), "Parser.frame")
				End If
				If Not File.Exists(fr) Then
			...
			If tab.nsName IsNot Nothing AndAlso tab.nsName.Length > 0 Then
				...
				indent += 1
			End If
			CopyFramePart("-->constants", indent)
			indent += 1
			GenTokens(indent)
