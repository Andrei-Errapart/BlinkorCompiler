...
Namespace at.jku.ssw.Coco
	...
	Public Class DFA
		...
		Private       strRemainder As String = Nothing
		'---------- Output primitives
		...
		Private Sub [Step](ByVal from As State, ByVal p As Node, ByVal stepped As BitArray)
			...
			Select Case p.typ
				...
				Case Node.iter, Node.opt
					...
					End If
					[Step](from, p.[sub], stepped)
					If (p.typ = Node.iter) AndAlso p.state IsNot from Then
		...
		End Sub
		Private Sub CopyFramePart(ByVal [stop] As String, ByVal indent As Integer)
			If Len(strRemainder) > 0 Then
				gen.WriteLine(strRemainder)
				strRemainder = ""
			End If
			While True
				Dim strLine As String = fram.ReadLine()
				If IsNothing(strLine) Then Exit While
				Dim spos As Integer = InStr(strLine, [Stop])
				If spos > 0 Then
					If Len([Stop]) = Len(strLine) Then
						Return
					Else
						strRemainder = Mid(strLine, spos + Len([stop]))
		...
		Private Sub WriteState(ByVal state As State, ByVal indent As Integer)
			...
			If ctxEnd Then
				...
				gen.WriteLine(StrDup(indent, vbTab) & "buffer.Pos = t.pos")
				gen.WriteLine(StrDup(indent, vbTab) & "NextCh()")
				gen.WriteLine(StrDup(indent, vbTab) & "line = t.line")
				gen.WriteLine(StrDup(indent, vbTab) & "col = t.col")
				gen.WriteLine(StrDup(indent, vbTab) & "For i As Integer = 0 To tlen - 1")
				gen.WriteLine(StrDup(indent + 1, vbTab) & "NextCh()")
				gen.WriteLine(StrDup(indent, vbTab) & "Next")
			End If
			If endOf Is Nothing Then
				gen.WriteLine(StrDup(indent, vbTab) & "t.kind = noSym")
			Else
		...
		End Sub
		Private Sub OpenGen(ByVal backUp As Boolean) ' pdt
			Try
				Dim fn As String = Path.Combine(tab.outDir, "Scanner.vb") ' pdt
				If File.Exists(fn) AndAlso backUp Then
		...
		Public Sub WriteScanner()
			...
			Dim fr As String = Path.Combine(tab.srcDir, "Scanner.frame") ' pdt
			If Not File.Exists(fr) Then
				If tab.frameDir IsNot Nothing Then
					fr = Path.Combine(tab.frameDir.Trim(), "Scanner.frame")
				End If
				If Not File.Exists(fr) Then
			...
			End If
			Try
				fram = New StreamReader(fr) 'FileStream(fr, FileMode.Open, FileAccess.Read, FileShare.Read)
			...
			If dirtyDFA Then
				MakeDeterministic()
			End If
			CopyFramePart("-->begin", indent)
			If Not tab.srcName.ToLower().EndsWith("coco.atg") Then
				gen.Close()
				OpenGen(False) ' pdt
			End If
			CopyFramePart("-->namespace", indent)
			If tab.nsName IsNot Nothing AndAlso tab.nsName.Length > 0 Then
				gen.WriteLine("Namespace {0}", tab.nsName)
				indent += 1
			End If
			CopyFramePart("-->declarations", indent)
			indent += 1
			...
			End If
			indent -= 1
			CopyFramePart("-->initialization", indent)
			WriteStartTab(indent + 2)
			CopyFramePart("-->casing1", indent)
			indent += 2
			If ignoreCase Then
